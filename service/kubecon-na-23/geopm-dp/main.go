...
func stubAllocFunc(r *pluginapi.AllocateRequest, devs map[string]pluginapi.Device) (*pluginapi.AllocateResponse, error) {
	...
	for _, req := range r.ContainerRequests {
		...
		response.Envs = map[string]string{
			"USER_POWER_CAP": fmt.Sprintf("%d", powerCap),
		}
...
func generateDevices(limit int) []*pluginapi.Device {
	devs := []*pluginapi.Device{}
	for i := 0; i < limit; i++ {
		devs = append(devs, &pluginapi.Device{ID: fmt.Sprintf("Dev-%d", i), Health: pluginapi.Healthy})
	}
	return devs
}
func main() {
	...
	dp1 := plugin.NewDevicePluginStub(generateDevices(1000), socketPath, resourceName, false, false)
