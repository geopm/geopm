...
def create_prof(shm_key, pid, uid, gid):
    global _dl
    err = _dl.geopm_shmem_create_prof(shm_key, pid, uid, gid)
    if err < 0:
...
def path_prof(shm_key, pid, uid, gid):
    ...
    err = _dl.geopm_shmem_path_prof(shm_key, pid, uid, name_max, result_cstr)
    if err < 0:
