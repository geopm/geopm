...
The save_control() method is implemented so that one must register all desired
...
taken to ensure all IOGroup plugins that the agent depends on are loaded prior
to calling init().
