...
'''
Helper functions for running feed forward neural net agent experiments.
'''
...
def launch_configs(output_dir, app_conf, cpu_nn_path):
    ...
    targets = []
    agent = 'ffnet'
    file_name = os.path.join(output_dir, '{}_agent.config'.format(agent))
    agent_conf = geopmpy.agent.AgentConf(file_name, agent, {})
    targets.append(launch_util.LaunchConfig(app_conf=app_conf,
                                            agent_conf=agent_conf,
                                            name="job"))

    if cpu_nn_path is not None:
        os.environ['GEOPM_NN_PATH'] = cpu_nn_path
