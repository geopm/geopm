...
signals and writing controls from system components.  This module
differs from geopmdpy.pio by including signals read from the GEOPM
Runtime IOGroups.
...
int geopm_pio_rt_signal_name(int name_idx,
                             size_t result_max,
                             char *result);

int geopm_pio_rt_num_control_name(void);

int geopm_pio_rt_control_name(int name_index,
                              size_t result_max,
                              char *result);
...
int geopm_pio_rt_read_signal(const char *signal_name,
                             int domain_type,
                             int domain_idx,
                             double *result);

int geopm_pio_rt_write_control(const char *control_name,
                               int domain_type,
                               int domain_idx,
                               double setting);

int geopm_pio_rt_push_signal(const char *signal_name,
                             int domain_type,
                             int domain_idx);

int geopm_pio_rt_push_control(const char *control_name,
                              int domain_type,
                              int domain_idx);

int geopm_pio_rt_sample(int signal_idx,
                        double *result);

int geopm_pio_rt_adjust(int control_idx,
                        double setting);
...
int geopm_pio_rt_signal_description(const char *signal_name,
                                    size_t description_max,
                                    char *description);

int geopm_pio_rt_control_description(const char *control_name,
                                     size_t description_max,
                                     char *description);
...
def signal_names():
    ...
    result = []
    num_signal = _dl.geopm_pio_rt_num_signal_name()
    if num_signal < 0:
        raise RuntimeError('geopm_pio_rt_num_signal_name() failed: {}'.format(error.message(num_signal)))
    name_max = 255
    signal_name_cstr = gffi.gffi.new("char[]", name_max)
    for signal_idx in range(num_signal):
        err = _dl.geopm_pio_rt_signal_name(signal_idx, name_max, signal_name_cstr)
        if err < 0:
            raise RuntimeError('geopm_pio_rt_signal_name() failed: {}'.format(error.message(err)))
...
def control_names():
    ...
    global _dl
    result = []
    num_control = _dl.geopm_pio_rt_num_control_name()
    if num_control < 0:
        raise RuntimeError('geopm_pio_rt_num_control_name() failed: {}'.format(error.message(num_control)))
    name_max = 255
    control_name_cstr = gffi.gffi.new("char[]", name_max)
    for control_idx in range(num_control):
        err = _dl.geopm_pio_rt_control_name(control_idx, name_max, control_name_cstr)
        if err < 0:
            raise RuntimeError('geopm_pio_rt_control_name() failed: {}'.format(error.message(err)))
...
def signal_domain_type(signal_name):
    ...
    result = _dl.geopm_pio_rt_signal_domain_type(signal_name_cstr)
    if result < 0:
        raise RuntimeError('geopm_pio_rt_signal_domain_type() failed: {}'.format(error.message(result)))
    return result
...
def control_domain_type(control_name):
    ...
    result = _dl.geopm_pio_rt_control_domain_type(control_name_cstr)
    if result < 0:
        raise RuntimeError('geopm_pio_rt_control_domain_type() failed: {}'.format(error.message(result)))
    return result
...
def read_signal(signal_name, domain_type, domain_idx):
    ...
    err = _dl.geopm_pio_rt_read_signal(signal_name_cstr, domain_type, domain_idx, result_cdbl)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_read_signal() failed: {}'.format(error.message(err)))
...
def write_control(control_name, domain_type, domain_idx, setting):
    ...
    control_name_cstr = gffi.gffi.new("char[]", control_name.encode())
    domain_type = topo.domain_type(domain_type)
    err = _dl.geopm_pio_rt_write_control(control_name_cstr, domain_type, domain_idx, setting)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_write_control() failed: {}'.format(error.message(err)))
...
def push_signal(signal_name, domain_type, domain_idx):
    ...
    signal_name_cstr = gffi.gffi.new("char[]", signal_name.encode())
    domain_type = topo.domain_type(domain_type)
    result = _dl.geopm_pio_rt_push_signal(signal_name_cstr, domain_type, domain_idx)
    if result < 0:
        raise RuntimeError('geopm_pio_rt_push_signal() failed: {}'.format(error.message(result)))
    return result
...
def push_control(control_name, domain_type, domain_idx):
    ...
    control_name_cstr = gffi.gffi.new("char[]", control_name.encode())
    domain_type = topo.domain_type(domain_type)
    result = _dl.geopm_pio_rt_push_control(control_name_cstr, domain_type, domain_idx)
    if result < 0:
        raise RuntimeError('geopm_pio_rt_push_control() failed: {}'.format(error.message(result)))
    return result
...
def sample(signal_idx):
    ...
    global _dl
    result_cdbl = gffi.gffi.new("double*")
    err = _dl.geopm_pio_rt_sample(signal_idx, result_cdbl)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_sample() failed: {}'.format(error.message(err)))
    return result_cdbl[0]
...
def adjust(control_idx, setting):
    ...
    """
    global _dl
    err = _dl.geopm_pio_rt_adjust(control_idx, setting)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_adjust() failed: {}'.format(error.message(err)))
...
def read_batch():
    ...
    """
    global _dl
    err = _dl.geopm_pio_rt_read_batch()
    if err < 0:
        raise RuntimeError('geopm_pio_rt_read_batch() failed: {}'.format(error.message(err)))
...
def write_batch():
    ...
    """
    global _dl
    err = _dl.geopm_pio_rt_write_batch()
    if err < 0:
        raise RuntimeError('geopm_pio_rt_write_batch() failed: {}'.format(error.message(err)))
...
def save_control():
    ...
    """
    global _dl
    err = _dl.geopm_pio_rt_save_control()
    if err < 0:
        raise RuntimeError('geopm_pio_rt_save_control() failed: {}'.format(error.message(err)))
...
def restore_control():
    ...
    global _dl
    err = _dl.geopm_pio_rt_restore_control()
    if err < 0:
        raise RuntimeError('geopm_pio_rt_restore_control() failed: {}'.format(error.message(err)))
...
def signal_description(signal_name):
    ...
    err = _dl.geopm_pio_rt_signal_description(signal_name_cstr, name_max, result_cstr)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_signal_description() failed: {}'.format(error.message(err)))
...
def control_description(control_name):
    ...
    control_name_cstr = gffi.gffi.new("char[]", control_name.encode())
    result_cstr = gffi.gffi.new("char[]", name_max)
    err = _dl.geopm_pio_rt_control_description(control_name_cstr, name_max, result_cstr)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_control_description() failed: {}'.format(error.message(err)))
    return gffi.gffi.string(result_cstr).decode()
...
def save_control_dir(save_dir):
    ...
    err = _dl.geopm_pio_rt_save_control_dir(save_dir_cstr)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_save_control_dir() failed: {}'.format(error.message(err)))
...
def restore_control_dir(save_dir):
    ...
    global _dl
    save_dir_cstr = gffi.gffi.new("char[]", save_dir.encode())
    err = _dl.geopm_pio_rt_restore_control_dir(save_dir_cstr)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_restore_control() failed: {}'.format(error.message(err)))
...
def signal_info(signal_name):
    ...
    err = _dl.geopm_pio_rt_signal_info(signal_name_cstr, aggregation_type, format_type, behavior_type)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_signal_info() failed: {}'.format(error.message(err)))
...
def start_batch_server(client_pid, signal_config, control_config):
    ...
    err = _dl.geopm_pio_rt_start_batch_server(client_pid,
                                           num_signal,
                                           signal_config_carr,
                                           num_control,
                                           control_config_carr,
                                           ...
                                           server_key_cstr)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_start_batch_server() failed: {}'.format(error.message(err)))
...
z                          is the value of the first element of the
    ...
    """
    err = _dl.geopm_pio_rt_stop_batch_server(server_pid)
    if err < 0:
        raise RuntimeError('geopm_pio_rt_stop_batch_server() failed: {}'.format(error.message(err)))

def update():
    """Update application data

    Raises:
        RuntimeError: Failure to update.

    """
    err = _dl.geopm_pio_rt_update()
    if err < 0:
        raise RuntimeError('geopm_pio_rt_update() failed: {}'.format(error.message(err)))
