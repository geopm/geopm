...
- [x] **DOC-2. Fill documentation gaps discovered while writing the skills.**
  ...
  **Eight more candidates found running a real energy-efficiency campaign**
  (single-node OpenMP STREAM, CentOS Stream 9, 2-socket Xeon 6978P, no GEOPM
  previously installed) by following the install and optimize skills exactly.
  Each cost a detour into `geopmdpy` source
  (`service.py`/`grid.py`/`optimizer.py`) or manual
  `geopmread`/`geopmaccess`/`ps` diagnosis that the collateral should have
  prevented. Recorded in
  [proposed_agent_changes.md](proposed_agent_changes.md); **all eight are now
  fixed** in the skill/script collateral (not `docs/source/`, since every one
  is specific to the assistants' scripts and references rather than the
  installed man pages):

  1. **Missing `python3-devel`/`pyconfig.h` prerequisite.** Added a row (and a
     Fedora/RHEL/Rocky/CentOS package column) to
     [client-venv.md](.github/skills/geopm-install/references/client-venv.md)'s
     prerequisites table, with a one-line check and the exact `pyconfig.h`
     error text to recognize.

  2. **`CPU_FREQUENCY_GOVERNOR_CONTROL` missing from the default access
     grant.** Added to `DEFAULT_CONTROLS` in
     [geopm-gen-access.sh](.github/skills/geopm-install/scripts/geopm-gen-access.sh)
     and to the "What a campaign needs" table in
     [access-lists.md](.github/skills/geopm-install/references/access-lists.md),
     with an explanation of why it is required unconditionally whenever
     `cpu-freq` is swept.

  3. **`cpu-power` needs `POWERCAP::CPU_POWER_LIMIT`, not
     `CPU_POWER_LIMIT_CONTROL`.** Fixed in all three places that had drifted:
     `geopm-gen-access.sh`'s `DEFAULT_CONTROLS`, `access-lists.md`'s table, and
     `geopm-sensitivity.sh`'s dimension-to-control `case` statement. Verified
     on real hardware: granting `POWERCAP::CPU_POWER_LIMIT` changed
     `--list-controls`'s `cpu-power` row from domain `n/a` to a real
     `package`-domain range. Deriving all three from one shared source (e.g.
     `grid.py`) so `gpu-freq`/`gpu-power`/`board-power` cannot drift the same
     way is a good follow-up but out of scope here.

  4. **`geopm-check-workload.sh`'s baseline measured under different
     conditions than the campaign.** Extended the script with `--dimension DIM`
     (and `--venv DIR`), which mirrors `geopm-sensitivity.sh`'s
     governor-forcing and sticker-capping for `cpu-freq` so the reported
     baseline is a valid reference point for the campaign that follows.
     Verified on real hardware: the STREAM baseline went from 69.9 s
     (unconstrained) to 99.9 s (`--dimension cpu-freq`, forced `performance`,
     capped at the sticker) — matching the sensitivity script's own reference
     point to within run-to-run noise. Default behavior (no `--dimension`) is
     unchanged and was re-verified.

  5. **`uncore-freq`'s current-value bound can resolve to a hard zero.**
     Reworded the relevant bullet in
     [sweep-dimensions.md](.github/skills/geopm-optimize/references/sweep-dimensions.md)
     to state the zero case explicitly, note it may be a firmware/BIOS default
     rather than a prior user's setting, and point at an administrator rather
     than implying it is client-side recoverable.

  6. **`geopm-sensitivity.sh` remedy #1 can suggest an invalid step.** Rewrote
     the coarsening calculation to search for the smallest divisor of the
     platform's actual grid-point count that is `>= ` the steps needed, so the
     printed override always evenly divides `(max - min)`. Added an explicit
     "NOT POSSIBLE by an even division alone" message for the degenerate case
     (the point count is prime, as it was on the test host: 13), rather than
     printing a command `geopmopt` will reject.

  7. **No troubleshooting entry for a stuck cross-session write-mode lock.**
     Added a full symptom/cause/fix entry to both `troubleshooting.md` copies
     (install and optimize skills), with the exact rule: `os.getsid()` ties the
     lock to the session leader, survives a `geopmd` restart, and is released
     only by a command from the same session completing or that session's
     leader exiting. **Confirmed twice more, live, while testing this exact
     fix** — the fix for gap #4 above was blocked by precisely this symptom
     against two different terminal sessions in a row, each resolved by
     closing the stale terminal per the new troubleshooting entry.

  8. **`--application-timeout` does not reliably kill the launched process
     tree.** Documented in `campaign-design.md` (Timeout section) and in
     `geopm-optimize/references/troubleshooting.md` ("Every trial times out"),
     cross-linked with item 7 since an orphaned workload process is one way the
     write-lock symptom above gets triggered.
