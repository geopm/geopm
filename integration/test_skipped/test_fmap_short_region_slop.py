...
class TestIntegration_fmap_short_region_slop(unittest.TestCase):
    ...
    def create_agent_conf(cls):
        ...
        """
        scaling_hash = [geopmdpy.hash.hash_str('scaling_{}'.format(idx))
                       for idx in range(cls._num_duration)]
        timed_hash = [geopmdpy.hash.hash_str('timed_{}'.format(idx))
                     for idx in range(cls._num_duration)]
        # Configure the agent
        ...
        agent_conf_fixed_dict = {'FREQ_CPU_DEFAULT':freq_max}
    ...
    def test_generate_trace_plot(self):
        ...
        for region_idx in range(self._num_duration):
            ...
            begin_region_hash = geopmdpy.hash.hash_str(scaling_region_name)
            timed_region_name = 'timed_{}'.format(region_idx)
            end_region_hash = geopmdpy.hash.hash_str(timed_region_name)
...
def select_region(trace, region_prefix, extension=''):
    ...
    for ext in extension:
        ...
        region_hash.append(geopmdpy.hash.hash_str(region_name))
