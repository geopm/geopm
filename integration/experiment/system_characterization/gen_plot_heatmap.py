...
def plot_heatmap(data, cmap, norm, zbar_range, zbar_label, z_thresh,
    ...
    ax.xaxis.set_major_locator(MaxNLocator(integer=True, prune='both'))  # Adjust x-axis ticks
    ax.yaxis.set_major_locator(MaxNLocator(integer=True, prune='both'))  # Adjust y-axis ticks
...
def setup_3d_data(df, metric, show_details, scalar=1e9):
    ...
    df['CPU_FREQUENCY_MIN_CONTROL@package-0'] /= scalar
    df['CPU_UNCORE_FREQUENCY_MIN_CONTROL@package-0'] /= scalar
    xs = sorted(df['CPU_FREQUENCY_MIN_CONTROL@package-0'].unique())
    ys = sorted(df['CPU_UNCORE_FREQUENCY_MIN_CONTROL@package-0'].unique())
    ...
    for xx in xs:
        temp = []
        for yy in ys:
            value = df.loc[df['CPU_FREQUENCY_MIN_CONTROL@package-0'] == xx].loc[df['CPU_UNCORE_FREQUENCY_MIN_CONTROL@package-0'] == yy]
...
if __name__ == '__main__':
    ...
    regex_report_format = f"*boardcap_{args.boardcap}_*_trial_1_*{args.hostname}*report"
