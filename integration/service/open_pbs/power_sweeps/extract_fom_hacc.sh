...
#
# Usage:
#   extract_fom_hacc.sh <dir> [<dir> ...]
#       Process all .log files in each given directory.
#
#   extract_fom_hacc.sh <log_file> <report_file>
...
extract_fom() {
    ...
    local fom
    fom=$(awk '/^step/ { steptime=$3 } /\^3 grid$/ { particles=int($1)^3 } END { if (steptime > 0) print particles/steptime; else print "NA" }' "$log_file")

    if [[ "$fom" == "NA" || -z "$fom" ]]; then
