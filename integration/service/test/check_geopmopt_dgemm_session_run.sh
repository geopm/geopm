...
trap 'rm -f "${BENCH_CONF}" "${SIGNAL_CONFIG}" "${CONTROL_CONFIG}"; rm -f "${DAEMON_PID_FILE}"' EXIT
...
kill $(cat ${DAEMON_PID_FILE}); tail -f /dev/null --pid $(cat ${DAEMON_PID_FILE}); rm -f ${DAEMON_PID_FILE}
...
energy_total = 0.0
host_count = 0
with open('test_geopm_dgemm_session_report.yaml') as fid:
    rpt = safe_load(fid)
energy_total += rpt['metrics']['CPU_ENERGY']['last'] - rpt['metrics']['CPU_ENERGY']['first']
