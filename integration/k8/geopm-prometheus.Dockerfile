...
RUN apt-get update && apt-get install -yq --no-install-recommends gpg wget
...
RUN apt-get update && apt-get install -yq --no-install-recommends software-properties-common python3-pip
RUN python3 -m pip install --ignore-installed grpcio==1.47.5
COPY geopm-prometheus /mnt/geopm-prometheus
RUN apt-get install -yq --no-install-recommends /mnt/geopm-prometheus/*.deb
RUN printf \
