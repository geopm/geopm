...
RUN dnf -y update && \
    dnf install -y autoconf automake gcc-c++ glibc-devel gmock-devel gtest-devel \
                   ...
                   python3-protobuf python3-sdnotify && \
    mkdir -p /mnt/geopm-fedora && \
    chmod a+rwx /mnt/geopm-fedora && \
    useradd -m build

USER build
ENV PACKAGING_URL=https://raw.githubusercontent.com/geopm/geopm/refs/heads/release-v3.2/release/fedora
WORKDIR /home/build
RUN mkdir -p rpmbuild/SOURCES && \
    mkdir -p rpmbuild/SPECS && \
    curl -sL https://github.com/geopm/geopm/archive/v3.2.1/geopm-3.2.1.tar.gz > \
        rpmbuild/SOURCES/geopm-3.2.1.tar.gz && \
...
USER root
RUN dnf install -y rpmbuild/RPMS/*/libgeopmd-3.2.1*.rpm rpmbuild/RPMS/*/libgeopmd-devel-3.2.1*.rpm

USER build
RUN rpmbuild -ba rpmbuild/SPECS/geopmd.spec && \
    pushd geopm-3.2.1/geopmdrs && \
    curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y && \
    export PATH="$HOME/.cargo/bin:${PATH}" && \
    sed -e "s|@VERSION@|3.2.1|" Cargo.toml.in > Cargo.toml && \
    cargo vendor && \
    cargo build --release && \
...
USER root
RUN dnf install -y rpmbuild/RPMS/*/libgeopm-3.2.1*.rpm rpmbuild/RPMS/*/libgeopm-devel-3.2.1*.rpm \
                   rpmbuild/RPMS/*/python3-geopmdpy-3.2.1*.rpm rpmbuild/RPMS/*/geopmd-cli-3.2.1*.rpm
...
RUN dnf update -y && \
    dnf install --setopt=install_weak_deps=False -y util-linux  \
                                                    /mnt/geopm-fedora/libgeopmd-3.2.1*.rpm \
                                                    /mnt/geopm-fedora/geopmd-cli-3.2.1*.rpm \
                                                    /mnt/geopm-fedora/python3-geopmdpy-3.2.1*.rpm && \
    dnf clean all && \
