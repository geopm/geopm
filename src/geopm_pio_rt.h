...
#ifndef GEOPM_PIO_RT_H_INCLUDE
#define GEOPM_PIO_RT_H_INCLUDE
...
///         name_idx parameter to the geopm_pio_rt_signal_name()
...
///        geopm_pio_rt_num_signal_name() or else an error will occur.
///
...
///         name_idx parameter to the geopm_pio_rt_control_name()
///         function.  Any error in loading the platform will result
///         in a negative error code describing the failure.
...
/// @param [in] name_idx The value of name_idx must be greater than
///        zero and less than the return value from
///        geopm_pio_rt_num_control_name() or else an error will occur.
///
/// @param [in] result_max At most result_max bytes are written to the
///        result string. If result_max is too small to contain the
...
///          calling geopm_pio_rt_read_batch().
///
...
///        the values returned by the geopm_pio_rt_signal_name()
...
///
/// @details Subsequent calls to the geopm_pio_rt_read_batch() function
...
///          geopm_pio_rt_read_batch() or geopm_pio_rt_adjust().  After
///          calls to geopm_pio_rt_read_batch() or geopm_pio_rt_adjust()
///          have been made, signals may be pushed again only after
///          performing a reset by calling geopm_pio_rt_reset() and
///          before calling geopm_pio_rt_read_batch() or
///          geopm_pio_rt_adjust() again.  Attempts to push a signal
///          onto the stack after the first call to
///          geopm_pio_rt_read_batch() or geopm_pio_rt_adjust() (and
///          without performing a reset) or attempts to push a
///          signal_name that is not a value provided by
///          geopm_pio_rt_signal_name() will result in a negative return
///          value.
///
/// @param [in] signal_name The name of the signal, coming from the
///        geopm_pio_rt_signal_name() function.
///
...
///
/// @return The return value of geopm_pio_rt_push_signal() is an index
///         that can be passed as the sample_idx parameter to
///         geopm_pio_rt_sample() to access the signal value stored in
...
///        of the values returned by the geopm_pio_rt_control_name()
///        function, the domain_type from one of the geopm_domain_e
///        values, and the domain_idx between zero to the value
///        returned by geopm_topo_num_domain(domain_type).
///
/// @details Subsequent calls to the geopm_pio_rt_write_batch() function
...
///          geopm_pio_rt_adjust() or geopm_pio_rt_read_batch().  After
///          calls to geopm_pio_rt_adjust() or geopm_pio_rt_read_batch()
///          have been made, controls may be pushed again only after
///          performing a reset by calling geopm_pio_rt_reset() and
///          before calling geopm_pio_rt_adjust() or
///          geopm_pio_rt_read_batch() again.  Attempts to push a
///          control onto the stack after the first call to
///          geopm_pio_rt_adjust() or geopm_pio_rt_read_batch() (and
///          without performing a reset) or attempts to push a
///          control_name that is not a value provided by
///          geopm_pio_rt_control_name() will result in a negative return
///          value.
///
/// @param [in] control_name The name of the control, coming from the
///        geopm_pio_rt_control_name() function.
///
/// @param [in] domain_type The type of the domain from one of the
///        geopm_domain_e values
///
...
///        geopm_topo_num_domain(domain_type)
///
/// @return The return value of geopm_pio_rt_push_control() can be passed
///         to the geopm_pio_rt_adjust() function which will update the
...
///        pushed via geopm_pio_rt_push_signal() and writes the value
///        into result.
///
/// @details The cached value is updated at the time of call to
///          geopm_pio_rt_read_batch().
///
/// @param [in] signal_idx The signal_idx provided matches the return
///        value from geopm_pio_rt_push_signal() when the signal was
///        pushed.
///
/// @param [out] result The value of the signal.
///
...
///        pushed via geopm_pio_rt_push_control() to the value setting.
///
/// @details The cached value will be written to the platform at time
///          of call to geopm_pio_rt_write_batch().
///
/// @param [in] control_idx The control_idx provided matches the
///        return value from geopm_pio_rt_push_control() when the control
///        was pushed.
///
...
///        call to geopm_pio_rt_sample() will reflect the updated data.
///
/// @return If an error occurs then negative error code is returned.
///         Zero is returned upon success.
...
///        geopm_pio_rt_adjust() are written to the platform.
///
/// @return If an error occurs then negative error code is returned.
///         Zero is returned upon success.
...
///        changes made through geopm_pio_rt_write_control() or
///        geopm_pio_rt_write_batch() may be reverted with a call to
///        geopm_pio_rt_restore_control().
///
...
///        subsequent changes made through geopm_pio_rt_write_control()
///        or geopm_pio_rt_write_batch() may be reverted with a call to
///        geopm_pio_rt_restore_control().
///
/// @details The control settings are stored in memory managed by
///          GEOPM.
...
///        geopm_pio_rt_save_control() so that all subsequent changes
///        made through geopm_pio_rt_write_control or
///        geopm_pio_rt_write_batch() are reverted to their previous
///        settings.
///
/// @return If an error occurs then negative error code is returned.
///         Zero is returned upon success.
...
/// @brief Restore the state recorded by the last call to
///        geopm_pio_rt_save_control() in the directory so that all
///        subsequent changes made through geopm_pio_rt_write_control or
///        geopm_pio_rt_write_batch() are reverted to their previous
///        settings.
///
...
///        the application.
int geopm_pio_rt_profile_pids(const char *profile_name, int max_num_pid, int *num_pid, int *pid);
