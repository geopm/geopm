...
namespace geopm
{
    ...
    /// bitfields.
    class VariorumFieldControl : public Control
    {
        public:
            VariorumFieldControl(std::shared_ptr<MSRIO> msrio,
                            int cpu,
                            uint64_t offset,
                            int begin_bit,
                            int end_bit,
                            int function,
                            double scalar);
            VariorumFieldControl(const VariorumFieldControl &other) = delete;
            virtual ~VariorumFieldControl() = default;
