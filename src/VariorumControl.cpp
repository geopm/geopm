...
#include "VariorumFieldControl.hpp"
#include "VariorumIO.hpp"
#include "Variorum.hpp"
#include "Exception.hpp"
#include "geopm_debug.hpp"

namespace geopm
{
    VariorumFieldControl::VariorumFieldControl(std::shared_ptr<MSRIO> msrio,
                                     int cpu,
                                     uint64_t offset,
                                     int begin_bit,
                                     int end_bit,
    ...
    {
        if (m_msrio == nullptr) {
            throw Exception("VariorumFieldControl: cannot construct with null MSRIO",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        if (m_function < 0 || m_function >= MSR::M_FUNCTION_OVERFLOW) {
            throw Exception("VariorumFieldControl: unsupported encode function.",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        if (begin_bit > end_bit) {
            throw Exception("VariorumFieldControl: begin bit must be <= end bit",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
    }

    void VariorumFieldControl::setup_batch(void)
    {
    ...
    uint64_t VariorumFieldControl::encode(double value) const
    {
        uint64_t result = 0;
        ...
        switch (m_function) {
            ...
            case MSR::M_FUNCTION_7_BIT_FLOAT:
                ...
                if (value > 0) {
                    ...
                    throw Exception("VariorumFieldControl::encode(): input value <= 0 for M_FUNCTION_7_BIT_FLOAT",
                                    GEOPM_ERROR_INVALID, __FILE__, __LINE__);
                }
                break;
            default:
    ...
    void VariorumFieldControl::adjust(double value)
    {
        GEOPM_DEBUG_ASSERT(m_msrio != nullptr, "null MSRIO");
        if (!m_is_batch_ready) {
            throw Exception("VariorumFieldControl::adjust(): cannot call adjust() before setup_batch()",
                            GEOPM_ERROR_RUNTIME, __FILE__, __LINE__);
        }
        m_msrio->adjust(m_adjust_idx, encode(value), m_mask);
    }

    void VariorumFieldControl::write(double value)
    {
        GEOPM_DEBUG_ASSERT(m_msrio != nullptr, "null MSRIO");
        m_msrio->write_msr(m_cpu, m_offset, encode(value), m_mask);
    }

    void VariorumFieldControl::save(void)
    {
        m_saved_msr_value = m_msrio->read_msr(m_cpu, m_offset);
        m_saved_msr_value &= m_mask;
    }

    void VariorumFieldControl::restore(void)
    {
