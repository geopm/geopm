...
#include "Helper.hpp"
#include "VariorumPath.hpp"

#define GEOPM_IOC_Variorum_BATCH _IOWR('c', 0xA2, struct geopm::VariorumIOImp::m_msr_batch_array_s)

namespace geopm
{
    std::unique_ptr<VariorumIO> VariorumIO::make_unique(void)
    {
        return geopm::make_unique<VariorumIOImp>();
    }

    std::shared_ptr<VariorumIO> VariorumIO::make_shared(void)
    {
        return std::make_shared<VariorumIOImp>();
    }

    VariorumIOImp::VariorumIOImp()
        : VariorumIOImp(geopm_sched_num_cpu(), std::make_shared<VariorumPath>())
    {

    }

    VariorumIOImp::VariorumIOImp(int num_cpu, std::shared_ptr<VariorumPath> path)
        : m_num_cpu(num_cpu)
    ...
    void VariorumIOImp::open_all(void)
    {
        if (!m_is_open) {
    ...
    void VariorumIOImp::close_all(void)
    {
        if (m_is_open) {
            close_msr_batch();
    ...
    uint64_t VariorumIOImp::read_msr(int cpu_idx,
                                uint64_t offset)
    {
        uint64_t result = 0;
        ...
        if (num_read != sizeof(result)) {
            ...
            err_str << "VariorumIOImp::read_msr(): pread() failed at offset 0x" << std::hex << offset
                    << " system error: " << strerror(errno);
            throw Exception(err_str.str(), GEOPM_ERROR_Variorum_READ, __FILE__, __LINE__);
        }
        return result;
    }

    void VariorumIOImp::write_msr(int cpu_idx,
                             uint64_t offset,
                             uint64_t raw_value,
                             uint64_t write_mask)
    {
        if ((raw_value & write_mask) != raw_value) {
            std::ostringstream err_str;
            err_str << "VariorumIOImp::write_msr(): raw_value does not obey write_mask, "
        ...
        if (num_write != sizeof(write_value)) {
            std::ostringstream err_str;
            err_str << "VariorumIOImp::write_msr(): pwrite() failed at offset 0x" << std::hex << offset
                    << " system error: " << strerror(errno);
            throw Exception(err_str.str(), GEOPM_ERROR_Variorum_WRITE, __FILE__, __LINE__);
        }
    }

    uint64_t VariorumIOImp::system_write_mask(uint64_t offset)
    {
        ...
        if (off_it != m_offset_mask_map.end()) {
            ...
            int err = ioctl(msr_batch_desc(), GEOPM_IOC_Variorum_BATCH, &arr);
            if (err || wr.err) {
                throw Exception("VariorumIOImp::system_write_mask(): read of mask failed",
    ...
    int VariorumIOImp::add_write(int cpu_idx, uint64_t offset)
    {
        int result = -1;
    ...
    void VariorumIOImp::adjust(int batch_idx, uint64_t raw_value, uint64_t write_mask)
    {
        if (batch_idx < 0 || (size_t)batch_idx >= m_write_batch_op.size()) {
            throw Exception("VariorumIOImp::adjust(): batch_idx out of range: " + std::to_string(batch_idx),
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        ...
        if ((~wmask_sys & write_mask) != 0ULL) {
            throw Exception("VariorumIOImp::adjust(): write_mask is out of bounds",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        if ((raw_value & write_mask) != raw_value) {
            std::ostringstream err_str;
            err_str << "VariorumIOImp::adjust(): raw_value does not obey write_mask, "
                    << "raw_value=0x" << std::hex << raw_value
                    << " write_mask=0x" << write_mask;
            throw Exception(err_str.str(), GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
    ...
    int VariorumIOImp::add_read(int cpu_idx, uint64_t offset)
    {
    ...
    uint64_t VariorumIOImp::sample(int batch_idx) const
    {
        if (!m_is_batch_read) {
            throw Exception("VariorumIOImp::sample(): cannot call sample() before read_batch().",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
    ...
    void VariorumIOImp::msr_ioctl(struct m_msr_batch_array_s &batch)
    {
        int err = ioctl(msr_batch_desc(), GEOPM_IOC_Variorum_BATCH, &batch);
        if (err) {
            err = errno ? errno : err;
            std::ostringstream err_str;
            err_str << "VariorumIOImp::msr_ioctl(): call to ioctl() for /dev/cpu/msr_batch failed: "
        ...
        for (uint32_t batch_idx = 0; batch_idx != batch.numops; ++batch_idx) {
            ...
            if (err) {
                ...
                std::ostringstream err_str;
                err_str << "VariorumIOImp::msr_ioctl(): operation failed at offset 0x"
                        << std::hex << offset
                        << " system error: " << strerror(err);
                throw Exception(err_str.str(), GEOPM_ERROR_Variorum_WRITE, __FILE__, __LINE__);
            }
   ...
   void VariorumIOImp::msr_ioctl_read(void)
    {
        GEOPM_DEBUG_ASSERT(m_read_batch.numops == m_read_batch_op.size() &&
                           m_read_batch.ops == m_read_batch_op.data(),
                           "Batch operations not updated prior to calling VariorumIOImp::msr_ioctl_read()");
        msr_ioctl(m_read_batch);
    }

    void VariorumIOImp::msr_ioctl_write(void)
    {
        GEOPM_DEBUG_ASSERT(m_write_batch.numops == m_write_batch_op.size() &&
                           m_write_batch.ops == m_write_batch_op.data(),
                           "Batch operations not updated prior to calling VariorumIOImp::msr_ioctl_write()");
    ...
    void VariorumIOImp::read_batch(void)
    {
    ...
    void VariorumIOImp::write_batch(void)
    {
    ...
    int VariorumIOImp::msr_desc(int cpu_idx)
    {
        if (cpu_idx < 0 || cpu_idx > m_num_cpu) {
            throw Exception("VariorumIOImp::msr_desc(): cpu_idx=" + std::to_string(cpu_idx) +
    ...
    int VariorumIOImp::msr_batch_desc()
    {
        return m_file_desc[m_num_cpu];
    }

    void VariorumIOImp::open_msr(int cpu_idx)
    {
        ...
        if (err) {
            throw Exception("VariorumIOImp::open_msr(): file descriptor invalid",
                            GEOPM_ERROR_Variorum_OPEN, __FILE__, __LINE__);
        }
    }

    void VariorumIOImp::open_msr_batch(void)
    {
        ...
        if (m_is_batch_enabled) {
            ...
            if (err) {
                throw Exception("VariorumIOImp::open_msr_batch(): file descriptor invalid",
                                GEOPM_ERROR_Variorum_OPEN, __FILE__, __LINE__);
            }
        }
    }

    void VariorumIOImp::close_msr(int cpu_idx)
    {
    ...
    void VariorumIOImp::close_msr_batch(void)
    {
        if (m_file_desc[m_num_cpu] != -1) {
